...
from boss import (WitheredKing, BossRoomCutscene, WitheredKingHealthBar,
                   draw_win_screen, draw_garden_arena_background)
...
def main():
    ...
    def enter_level4(player_obj):
        # Ancient Garden Arena - intro cutscene + final boss
        ...
        camera_locked = True   # fixed arena, no scrolling
        current_wave = 0
        level_progress = 0
        enemies.empty()
        ...
        level4_cutscene = BossRoomCutscene(walk_target_x=SCREEN_WIDTH * 0.3,
    ...
    while running:
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                ...
                # Member 4 shortcut: jump directly into Level 4 (final boss).
                if event.key in (pygame.K_4, pygame.K_KP4) and game_state in ("TITLE", "PLAYING"):
                    if player is None:
                        player = Player(100, GROUND_Y - 128)
                    game_state = "PLAYING"
                    show_tutorial = False
        ...
        if game_state == "COMET":
            ...
            if player:
                if current_level == 4 and level4_cutscene and level4_cutscene.player_locked:
                    # let update() run for gravity/animation, then cancel any
                    # keyboard movement so the scripted walk-in stays in control
                    pre_x = player.rect.x
                    player.update()
                    player.rect.x = pre_x
                else:
                ...
                # Member 4: Level 4 - Ancient Garden Arena (final boss)
                if current_level == 4 and level4_boss is not None:
                    ...
                    for proj in list(boss_projectiles):
                    ...
                    if level4_boss.state in (WitheredKing.PHASE1, WitheredKing.PHASE2):
                        if player.is_attacking:
                            ...
                            if hitbox.colliderect(level4_boss.rect):
                                ...
                                particle_emitter.emit(level4_boss.rect.centerx, level4_boss.rect.centery,
                                                        count=15, color=(120, 200, 140), lifetime=20,
                                                        size=3, speed_range=(1, 3))
                        for seed in player.seed_shots[:]:
                            if seed.rect.colliderect(level4_boss.rect):
                                level4_boss.take_damage(SEED_SHOT_DAMAGE)
                                particle_emitter.emit(seed.rect.centerx, seed.rect.centery,
                                                        count=15, color=(120, 200, 140), lifetime=20,
                                                        size=3, speed_range=(1, 3))
                    ...
                    if level4_boss.state == WitheredKing.DEAD and getattr(level4_boss, "death_complete", False):
                        score += 100
                        game_state = "WIN"
        ...
        if game_state == "TITLE":
            ...
            if current_level == 4:
                # Member 4: draw the arena to an offscreen surface so shake can move it as one unit
            ...
            if player:
                # Member 4: Withered King, boss room cutscene, projectiles, particles.
                if current_level == 4 and level4_boss is not None:
                    if level4_cutscene:
            ...
            # Member 4: victory screen, Petal Storm particles still falling
        ...
        # 3. Member 4: Withered King health bar
